...
import { useActiveProject, useDeleteProject } from '../../hook/useModifyProject'
import queryKeys from 'src/config/queryKeys'
import useRefreshQuery from 'src/hook/useRefreshQuery'

export default function ProjectComponentArchived({ project }: { project: ProjectType }) {
  const [anchorElMenu, setAnchorElMenu] = React.useState(null)
  const [showSetting, setShowSetting] = useState(false)
  const { activeProject } = useActiveProject()
  const { deleteProject } = useDeleteProject()
  const { refreshQuery } = useRefreshQuery([queryKeys.projects.listArchived])
  ...
  const handleActiveProject = () => {
    activeProject(project._id, refreshQuery)
    setAnchorElMenu(null)
  }

  const handleDeleteProject = async () => {
    deleteProject(project._id, refreshQuery)
    setAnchorElMenu(null)
  }
    ...
    <div className='w-full h-[200px] rounded-2xl shadow-lg bg-white p-5 flex flex-col justify-between'>
      <div className='flex gap-3'>
        ...
        <PopoverComponent anchorElement={anchorElMenu} handleClose={handleCloseUserMenu}>
          ...
          >
            <span className='hover:bg-primary-50'>Active project</span>
          </MenuItem>
          <MenuItem
            ...
            className='hover:bg-primary-50'
          >
            <span className='hover:bg-primary-50'>Delete project</span>
          </MenuItem>
        </PopoverComponent>
      </div>
      ...
      <SettingProject open={showSetting} handleClose={() => setShowSetting(false)} project={project}></SettingProject>
    </div>
  )
}
